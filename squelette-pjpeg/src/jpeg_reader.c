...
void read_SOF2(struct jpeg_desc *jpeg) 
    /* On lit l'en-tête SOF2 et on met les données utiles dans jpeg 
     *  le stream se trouve après le marqueur 0xffc2 */
{
    struct bitstream *stream = jpeg->stream;
    ...
    for (uint32_t k = 0; k < nombre_comp; k++) {
        ...
        erreur(nb_lu, 4, "Erreur lecture bitstream SOF2\n");
     ...
     * lu par le bitstream se trouve apres le marqueur 0xffc4 */
{
    ...
    uint16_t nb_byte_read = 0;
      (jpeg->tables_huffman)[indice][type] = load_huffman_table(stream, &nb_byte_read); //on load la table de huffman, cela peut aussi remplacer une table déjà existante
     ...
     * par le bitstream se trouve après le marqueur 0xffda */
{
    ...
    for (int k = 0; k < (int) nombre_composantes; k++) {
        ...
        uint32_t Se, Ss;
        nb_lu = read_bitstream(stream, 8, &Ss, false); //on lit le premier indice de la sélection spectrale
        erreur(nb_lu, 8, "Erreur lecture bitstream SOS\n");
        nb_lu = read_bitstream(stream, 8, &Se, false); //on lit le dernier indice de la sélection spectrale
        erreur(nb_lu, 8, "Erreur lecture bitstream SOS\n");
        ...
        uint32_t Ahl;
        nb_lu = read_bitstream(stream, 8, &Ahl, false);
        erreur(nb_lu, 8, "Erreur lecture bitstream SOS\n");
}


struct jpeg_desc *first_read_jpeg(const char *filename)
    /* On ouvre notre fichier jpeg et on
     * remplis la structure de données  pour la première fois*/
{
    ...
    if (marqueur != 0xffd8) { //on a pas un image au format jpeg
    ...
    read_SOS(jpeg); //le prochain bit lu du bitstream est alors une donnée brut de l'image pour le premier scan
    return jpeg;
}


void read_jpeg(struct jpeg_desc *jpeg)
    /* On a fini de lire un scan quelconque, on
     * lit les possibles nouvelles tables de huffman
     * puis on lit l'entête SOS*/
{

    uint32_t marqueur = 0;
    uint32_t nb_lu;
    nb_lu = read_bitstream(jpeg->stream, 16, &marqueur, false);
    erreur(nb_lu, 16, "Erreur lecture nouveau scan\n");
    ...
    while (marqueur != 0xffda) {
        switch (marqueur)
        {
            ...
            default:
                fprintf(stderr, "Marqueur inatendu\n");
                exit(1);
                break;
        }
...
uint8_t get_Ss(struct jpeg_desc *jpeg)
    /* on renvoie le premier indice de la sélection spectrale */
{
    return jpeg->Ss;
}

uint8_t get_Se(struct jpeg_desc *jpeg)
    /* on renvoie le dernier indice de la sélection spectrale */
{
    return jpeg->Se;
}
...
void close_jpeg(struct jpeg_desc *jpeg)
{
    for (int k = 0; k < 4; k++) {
        ...
        //libération tablles de quantification
