...
void remplacer_extension(const char *chaine,char *extension, int indice)
{
    char point = '.';
    int i=0;
    while (chaine[i]=='.')
        ...
        /* Chargement du fichier jpeg */
{
    ...
    struct jpeg_desc *jdesc = first_read_jpeg(filename);
    ...
    for (int32_t j = 0; j<nb_total_mcu; j++){    //On lit tous les blocs
      //printf("\n\n********* MCU %d ***********\n", j);
      p_mcu[j] = first_read_MCU(stream, jdesc, valeurs_prec);
    ...
    if (nb_Cb==0){
        ...
        char *ext = "pgm";
        remplacer_extension(filename, ext, 0);
    ...
    int compteur = 0;
    while (get_Se(jdesc)<63)
    {
        read_jpeg(jdesc); //Lecture du nouveau Scan
        compteur++;
        ...
        if (nb_Cb==0){
            printf("conversion pgm...\n");
            char *ext = "pgm";
            remplacer_extension(filename, ext, compteur);
            convert_pgm(p_mcu, jdesc, filename);
            //strcpy( command, "gnome-open ./resultat/image.pgm" );
            //system(command);
        }
