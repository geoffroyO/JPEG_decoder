...
bool ajouter_noeud_gauche( uint8_t valeur, uint8_t profondeur, struct huff_table *racine)
{
    ...
    if (profondeur == 1) {
        ...
        if (racine->fils_gauche == NULL) {
            ...
            racine->fils_droit = nouveau_noeud(valeur, true); //si déjà occupé à gauche on remplie à droite
            return true;
        }
    ...
    if (racine->fils_droit == NULL) { //si on est pas arrivé à insérer à gauche bifurque à droite
...
int8_t next_huffman_value(struct huff_table *table, struct bitstream *stream)
    /* Il suffis de parcourir l'arbre à gauche ou à droite
